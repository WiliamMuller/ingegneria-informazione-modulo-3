% homework_plot.m
% Reads homework.csv and appends the plot to homework_plot.pdf.
...
%% Ideal triangle wave
t_ideal = (0:N-1) / Fs;
t_mod   = mod(t_ideal, 1/f0);
x_ideal = zeros(1, N);
half_T  = 1 / (2 * f0);
for k = 1:N
    if t_mod(k) < half_T
        x_ideal(k) = (2 * A / (1/f0)) * t_mod(k);
    else
        x_ideal(k) = 2 * A - (2 * A / (1/f0)) * t_mod(k);
    end
end

%% Plot real part vs time
fig = figure('Name', sprintf('Triangle Wave L=%d', L), 'NumberTitle', 'off');
...
title(sprintf('Onda triangolare - Approssimazione con L=%d', L));
...
%% Export
out_svg = sprintf('homework_L%d_plot.svg', L);
